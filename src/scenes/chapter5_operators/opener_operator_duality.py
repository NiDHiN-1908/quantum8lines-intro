...
class Chapter5OperatorDuality(Scene):
    def construct(self):
        ...
        grid_L = NumberPlane(background_line_style={"stroke_opacity": 0.35}).shift(LEFT * 3)
        self.add(grid_L)
        ...
        self.play(grid_L.animate.apply_matrix(matrix), v.animate.apply_matrix(matrix), run_time=1.2)

        grid_R = NumberPlane(background_line_style={"stroke_opacity": 0.25}).shift(RIGHT * 3)
        f = Vector([1.5, -0.8], color=YELLOW).shift(RIGHT * 3)

        self.play(FadeIn(grid_R), GrowArrow(f), run_time=1.0)
        self.play(f.animate.apply_matrix([[1.2, 0.1], [0.3, 0.9]]), run_time=1.0)
